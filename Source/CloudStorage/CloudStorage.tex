...
Выбираем \mitem{Больше вариантов} \ $\ra$ \ \mitem{Использовать другую учётную запись} (если есть такое поле) и ставим флажок \mitem{Запомнить учётные данные}. Вводим данные от \hyperlink{lectory-operator-profile}{учётки оператора}.
...
Краткая инструкция, как примаунтить диск:
\begin{enumerate}
...
После этого поставить флаг \mitem{Redistered User}, ввести логин и пароль от owncloud. Флаг \mitem{Remember forever} позволит вам в будущем пропускать процесс ввода логина-пароля, но подключать диск после перезагрузки ПК всё равно придётся.
